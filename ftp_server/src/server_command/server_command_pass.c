...
int	server_command_pass(t_client_info *client_info, char *argument)
{
  ...
  if (!client_info->isAuthenticated && !client_info->username)
    return (REQUEST_RESPONSE(client_info->fd, SERVER_503), 0);
  if (client_info->isAuthenticated)
    return (REQUEST_RESPONSE(client_info->fd, SERVER_230), 0);
  ...
  if (!server_command_pass_fake_db_call(client_info->username, arguments[0]))
    client_info->isAuthenticated = true;
  if (client_info->isAuthenticated)
    REQUEST_RESPONSE(client_info->fd, SERVER_230);
  else
